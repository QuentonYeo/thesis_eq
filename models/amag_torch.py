...
try:
    ...
    _SCIPY = False
    warnings.warn(
        "SciPy not available; bandpass filtering will be skipped.", RuntimeWarning
    )
...
def crop_time_like(x: torch.Tensor, ref: torch.Tensor) -> torch.Tensor:
    ...
    if T < T_ref:
        ...
        pad = (
            ...
            0,
            T_ref - T,
        )  # (W_left,W_right, H_top,H_bottom, T_left,T_right) but for 3D it's slightly different.
...
class SelfAttention1D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn = torch.softmax(
            attn_scores, dim=-1
        )  # closest to keras-self-attention default
...
def _bandpass(
    data: np.ndarray, n: int = 2, f1: float = 1.0, f2: float = 20.0, dt: float = 0.01
) -> np.ndarray:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to HDF5 with /data/<trace_name> arrays",
    )
    parser.add_argument(
        "--csv_path",
        type=str,
        required=True,
        help="CSV with metadata (trace_name, source_magnitude, P index)",
    )
    parser.add_argument(
        "--save_dir", type=str, default="./out", help="Where to save model & outputs"
    )
    parser.add_argument("--depth", type=int, default=4)
    parser.add_argument("--kernel_size", type=int, default=5)
    parser.add_argument("--base_filters", type=int, default=8)
    parser.add_argument(
        "--loss", type=str, default="mse", choices=["mse", "wmse", "mse_mae"]
    )
    ...
    parser.add_argument(
        "--add_one",
        action="store_true",
        help="Label = magnitude + 1.0 (recommended to mirror Keras)",
    )
    ...
    parser.add_argument(
        "--random_window",
        action="store_true",
        help="Random pre-P window (like ratio mode).",
    )
    parser.add_argument(
        "--no_random_window", dest="random_window", action="store_false"
    )
    ...
    if "snr_db" in df.columns:
        ...
        try:
            df["snr_db"] = (
                df["snr_db"].astype(str).str.split(r"[\[\]\s\n]+").str[-2].astype(float)
            )
            snr_col = "snr_db"
        except Exception:
            pass
        mask = (
            (df.get("source_magnitude_type", "ml").str.lower() == "ml")
            & (df["source_magnitude"] > 0)
            & (df.get(snr_col, 100) > 10)
        )
    else:
        # INSTANCE-like
        mask = (
            (df.get("source_magnitude_type", "ML").str.upper() == "ML")
            & (df["source_magnitude"] > 0)
            & (df.get("trace_Z_snr_db", 100) > 10)
        )
