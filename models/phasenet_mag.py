...
class PhaseNet(WaveformModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if predict_magnitude:
            if magnitude_label not in phases:
                phases = phases + magnitude_label
        ...
        if self.predict_magnitude:
            self.magnitude_head = nn.Sequential(
                nn.Conv1d(last_filters, last_filters // 2, 1, padding="same"),
                nn.BatchNorm1d(last_filters // 2, eps=1e-3),
                nn.ReLU(),
                nn.Conv1d(last_filters // 2, 1, 1, padding="same"),
            )
    ...
    def forward(self, x, logits=False, return_magnitude=None):
        ...
        :param logits: If True, return raw logits for phase predictions
        :param return_magnitude: If True, return magnitude predictions. If None, uses self.predict_magnitude
        :return: Phase predictions and optionally magnitude predictions
        """
        ...
        # Phase predictions
        phase_out = self.out(x)
        if not logits:
            phase_out = self.softmax(phase_out)

        # Magnitude predictions
        if return_magnitude and hasattr(self, "magnitude_head"):
            magnitude_out = self.magnitude_head(x)
            return phase_out, magnitude_out
        else:
            return phase_out
    ...
    def annotate_batch_post(
        ...
        # Handle both phase and magnitude predictions
        if isinstance(batch, tuple):
            phase_batch, magnitude_batch = batch
            # Transpose predictions to correct shape
            phase_batch = torch.transpose(phase_batch, -1, -2)
            magnitude_batch = torch.transpose(magnitude_batch, -1, -2)

            prenan, postnan = argdict.get(
                "blinding", self._annotate_args.get("blinding")[1]
            )
            if prenan > 0:
                phase_batch[:, :prenan] = np.nan
                magnitude_batch[:, :prenan] = np.nan
            if postnan > 0:
                phase_batch[:, -postnan:] = np.nan
                magnitude_batch[:, -postnan:] = np.nan

            # Combine phase and magnitude predictions
            combined_batch = torch.cat([phase_batch, magnitude_batch], dim=-1)
            return combined_batch
        else:
            # Only phase predictions
    ...
    def classify_aggregate(self, annotations, argdict) -> sbu.ClassifyOutput:
        ...
        for phase in self.labels:
            if phase == "N":
                # Don't pick noise
                continue
            elif phase == self.magnitude_label and self.predict_magnitude:
                # Handle magnitude predictions differently - could add magnitude-based picking logic here
                continue
...
class PhaseNetLight(PhaseNet):
    ...
    def forward(self, x, logits=False, return_magnitude=None):
        ...
        final_features = torch.cat(
            [self.activation(self.bnu4(self.up4(x))), x_in], dim=1
        )

        # Phase predictions
        phase_out = self.out(final_features)
        if not logits:
            phase_out = self.softmax(phase_out)

        # Magnitude predictions
        if return_magnitude and hasattr(self, "magnitude_head"):
            magnitude_out = self.magnitude_head(final_features)
            return phase_out, magnitude_out
        else:
            return phase_out
...
class VariableLengthPhaseNet(PhaseNet):
    ...
    - Optional magnitude prediction head for predicting magnitude values at each timestep.
    ...
    def forward(self, x, logits=False, return_magnitude=None):
        ...
        features = self._get_final_features(x)

        # Phase predictions
        phase_out = self.out(features)
        if not logits:
            phase_out = self.output_activation(phase_out)

        # Magnitude predictions
        if (
            return_magnitude
            and hasattr(self, "magnitude_head")
            and self.magnitude_head is not None
        ):
            magnitude_out = self.magnitude_head(features)
            return phase_out, magnitude_out
        else:
            return phase_out
    ...
    def _get_final_features(self, x):
        """Get features from the last layer before output for both phase and magnitude prediction."""
        x = self.activation(self.in_bn(self.inc(x)))
